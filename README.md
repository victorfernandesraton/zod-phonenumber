...
- **IANA timezone support** — `.timezone('America/Bahia')` infers country/DDI and parses national numbers
...
Use an IANA timezone to infer the country and parse national numbers. The timezone map is generated during development from [`@vvo/tzdb`](https://www.npmjs.com/package/@vvo/tzdb) and bundled with the package.

```ts
const value = z.phone('America/Bahia').parse('71999927837')
value.countryCallingCode // '55'
value.number             // '+5571999927837'
```

The chainable form is also supported with `.timezone('America/Bahia')`:

```ts
const value = z.phone().timezone('America/Bahia').parse('71999927837')
value.number // '+5571999927837'
```

The timezone can be combined with `.ddd()`:

```ts
z.phone().timezone('America/Bahia').ddd('71').parse('71999927837') // ok
z.phone().timezone('America/Bahia').ddd('11').parse('71999927837') // throws
```

Technical zones such as `UTC` and `Etc/*`, invalid zones, and zones without a unique country are rejected. The timezone infers country/DDI only; it does not validate DDD on its own.
...
| `.timezone(zone)` | Infers country from an IANA timezone |

The constructor `z.phone()` accepts either a country code (`'BR'`) or an IANA timezone (`'America/Bahia'`). Values containing a `/` are treated as timezones.
...
# Regenerate the timezone map from @vvo/tzdb (dev dependency)
npm run generate:timezones

# Run tests with coverage report (65 tests)
npm test
...
The `test` and `test:all` scripts run the native Node.js test runner through `tsx` with coverage enabled. Tests exercise phone validation, import patterns, timezone resolution, and the timezone generator.

## License

`.number` is the complete E.164 value. Timezone mappings are generated by `scripts/generate-timezone-regions.ts` using the development-only [`@vvo/tzdb`](https://www.npmjs.com/package/@vvo/tzdb) dependency. The generated `timezone-regions.generated.ts` file is bundled; `@vvo/tzdb` is not required at runtime. Runtime identifiers are checked with [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat).
